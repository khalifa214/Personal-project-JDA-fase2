<x-layout>
    @if ($products != null)
        ...
        @if ($pages > 1)
            <div class="flex gap-0 flex-row shadow-product w-fit rounded-md">
                @if ($pages > 2) 
                    <a class="px-3 py-1 bg-gray-50 rounded-l-[5px] hover:bg-gray-200 border-r-[1px] border-x-gray-400" href="/kategori/{{ $id }}/{{ $name }}/1">&laquo;</a>
                @endif
                @if ($page > 1)
                    @php
                    $prev = $page - 1;
                    @endphp 
                    <a class="px-3 py-1 {{ (request()->is("kategori/$id/$name/$prev") ) ? "bg-gray-100" : "bg-grey-50"}} hover:bg-gray-200 border-x-[1px] border-x-gray-400" href="/kategori/{{ $id }}/{{ $name }}/{{ $prev }}">{{ $prev }}</a>
                @endif

                <a class="px-3 py-1 {{ (request()->is("kategori/$id/$name/$page") ) ? "bg-gray-100" : "bg-grey-50"}} hover:bg-gray-200 border-x-[1px] border-x-gray-400" href="/kategori/{{ $id }}/{{ $name }}/{{ $page }}">{{ $page }}</a>

                @if ($page < $pages)
                    @php
                    $next = $page + 1;
                    @endphp 
                    <a class="px-3 py-1 {{ (request()->is("kategori/$id/$name/$$next") ) ? "bg-gray-100" : "bg-grey-50"}} hover:bg-gray-200 border-x-[1px] border-x-gray-400" href="/kategori/{{ $id }}/{{ $name }}/{{ $next }}">{{ $next }}</a>
                @endif
                @if ($pages > 2 ) 
                    <a class="px-3 py-1 bg-gray-50 rounded-r-[5px] hover:bg-gray-200 border-l-[1px] border-x-gray-400" href="/kategori/{{ $id }}/{{ $name }}/{{ $pages }}">&raquo;</a>
                @endif
            </div>
        @endif 
    @endif
